...
class ItemLoader:
    ...
    def chunk_generator(self):
        ...
        for i in range(0, size, CHUNK_SIZE):
            # HF dataset use select instead of list[start:end]
    ...
    def load_in_parallel(self, workers):
        ...
        """
        results = []
        # Use // to return the floor integer. Because we're taking floor, we need to + 1
        chunk_count = (len(self.dataset) // CHUNK_SIZE) + 1
        with ProcessPoolExecutor(max_workers=workers) as pool:
            # Apply self.from_chunk function to each chunk from self.chunk_generator()
